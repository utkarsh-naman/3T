...
8. 1. Generate all the 8 possible canonical form [find in code](../src/TTTEngine/3TEngine.go#L79)
   ...
   5. But this will give the cell number to play in reverse, so subtract it by 10.</br> i.e 10 - ((index-1)%9+1) </br> $\because$ index = i+1</br> we can return 10 - (i%9+1) [find in code](../src/TTTEngine/3TEngine.go#L104-L114)